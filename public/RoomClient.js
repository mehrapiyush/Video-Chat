...
class RoomClient {
    ...
    constructor(localMediaEl, remoteVideoEl, remoteAudioEl, mediasoupClient, socket, room_id, name, successCallback,type,chatroom) {
        this.name = name
        ...
        this.createRoom(room_id).then(async function () {
            await this.join(name, room_id,type)
            this.initSockets()
            this._isOpen = true
            successCallback()
        }.bind(this))
    ...
    async join(name, room_id,type) {
        ...
        socket.request('join', {
            ...
            this.device = device
            await this.initTransports(device,type)
            this.socket.emit('getProducers',{type: type})
        }.bind(this)).catch(e => {
            console.log(e)
        })
    }
    ...
    async initTransports(device,type) {
...
//////////////////
    ...
    initSockets() {
        this.socket.on('consumerClosed', function ({
            ...
            this.removeConsumer(consumer_id, producer_socket_id)
             if(document.getElementById(producer_socket_id).getElementsByTagName('video').length===0)
        document.getElementById(producer_socket_id).remove()
        }.bind(this))
        ...
        this.socket.on('newProducers', async function (data) {
            ...
            for (let {
                    producer_id, producer_socket_id, producer_name
                } of data) {
    ...
    async consume(producer_id, producer_socket_id, producer_name) { 
        ...
        this.getConsumeStream(producer_id).then(function ({
            ...
            let elem, textElem;
           if (kind === 'video') {
                if(document.getElementById(producer_socket_id)===null){
                let d = document.createElement('div')
                d.className = "videostream"
                let d1= document.createElement('div')
                let d2= document.createElement('div')
                d.id = producer_socket_id
                d1.className= "screen"
                d2.className= "videouser"
                d.appendChild(d1)
                let t= document.createElement('p')
                
                t.innerHTML = producer_name
                d.appendChild(t)
                d.appendChild(d2)
                ...
                this.remoteVideoEl.appendChild(d)
                    ...
                    let d1= document.getElementById(producer_socket_id).getElementsByClassName('screen')[0]
                     let d2= document.getElementById(producer_socket_id).getElementsByClassName('videouser')[0]
                    elem = document.createElement('video')
                textElem = document.createElement('p')
                // consumer.log(currentLayers)
            ...
            consumer.on('trackended', function () {
                this.removeConsumer(consumer.id, producer_socket_id)
            }.bind(this))
    ...
    removeConsumer(consumer_id, producer_socket_id) {
        let elem = document.getElementById(consumer_id)
        console.log(elem+" "+consumer_id)
        elem.srcObject.getTracks().forEach(function (track) {
            track.stop()
        })
        elem.parentNode.removeChild(elem)
